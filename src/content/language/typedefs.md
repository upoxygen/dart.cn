...
类型别名常被叫做 _typedef_，因为它用关键字 `typedef` 声明。
它是引用某个类型的简短写法。
下面声明并使用名为 `IntList` 的类型别名：
...
2.13 之前，typedef 只能用于函数类型。
使用新的 typedef 至少需要 [语言版本][language version] 2.13。
:::
...
多数情况下，函数建议用[内联函数类型][inline function types]，而不是 typedef。
不过函数 typedef 仍然有用：
