...
同一个 Dart SDK 可以同时支持多个 Dart 语言版本。
编译器判断代码面向哪个版本，并按该版本解释代码。
...
少数情况下 Dart 会引入不兼容特性，例如[空安全][null safety]，语言版本这时就重要了。
出现破坏性变更时，原来能编译的代码可能不再能编译。
语言版本让你可以为每个库单独指定语言版本，以保持兼容。
...
就空安全而言，Dart SDK 2.12 到 2.19 允许你 _选择_ 是否把代码更新到空安全。
Dart 用语言版本让非空安全代码和空安全代码一起运行。
这样才有可能从非空安全迁移到空安全。
应用或 package 如何迁移到带不兼容特性的新语言版本，见
[大型 Dart 项目的渐进式空安全迁移][Gradual null safety migration for large Dart projects]。

Each package has a default language version equal to the **lower bound**
of the [SDK constraint][] in the `pubspec.yaml` file.

每个 package 的默认语言版本，等于 `pubspec.yaml` 里 [SDK 约束][SDK constraint] 的 **下界**。
...
**例如：** 下面这段 `pubspec.yaml` 表示该 package 默认使用 Dart 2.18 语言版本。
...
Dart 团队为何以及如何设计这套版本机制，见[语言版本规范][language versioning specification]。
...
Dart 发布时可能在语言版本后面加一个补丁号。
除了修 bug，补丁不应改变语言。
例如 Dart 2.18.3 是 Dart 2.18 SDK 语言版本的最新发布。
...
每个 Dart SDK 发布都支持自己的语言版本，以及同一主版本号下的全部先前版本。
也就是说 Dart SDK 2.18.3 支持语言版本 2.0 到 2.18（含），但不支持 Dart 1.x。
...
* Whenever a minor version of the SDK ships, a new language version appears.
  ...
  SDK 每次发布次版本，都会出现一个新的语言版本。
  实际上很多语言版本和前一版很像，彼此完全兼容。
  例如 Dart 2.9 语言和 Dart 2.8 语言非常接近。
...
* When a patch release of the SDK ships,
  ...
  SDK 发布补丁时，不能引入新的语言特性。
  例如 2.18.3 _仍然_ 是语言版本 2.18。
  它必须兼容 2.18.2、2.18.1 和 2.18.0。
...
默认情况下，一个 package 里的每个 Dart 文件使用相同的语言版本。
Dart 把 `pubspec.yaml` 中 SDK 约束的下界当作默认语言版本。
有时某个 Dart 文件需要更旧的语言版本。
例如你可能无法同时把 package 里的所有文件迁移到空安全。
...
Dart 支持按库选择语言版本。
要让某个 [Dart 库][Dart library] 使用和 package 其余部分不同的语言版本，
必须包含如下格式的注释：
...
`@dart` 必须写在 `//` 注释里（不能是 `///` 或 `/*`），
并且必须出现在文件中任何 Dart 代码之前。
空白（制表符和空格）不影响，
但 `@dart` 和版本字符串内部除外。
如上面的例子，`@dart` 注释之前可以有其他注释。
...
语言版本帮助 Dart 及其工具随时间演进。
除了可能破坏语法和语义的语言变更，
新特性和工具变更也可能被语言版本挡住。
例如 `dart format` 可能按所格式化库的语言版本，用不同方式格式化 Dart 代码。
...
每个语言版本引入了什么，可以查可筛选的 [Dart 更新日志][Dart changelog]。
每个语言版本及其特性的具体细节，见[语言文档][language documentation]和[语言规范][language specification]。
...
想看正在讨论、设计和实现的新 Dart 特性和变更，
去 GitHub 上看 [Dart language funnel][] 项目。
:::
